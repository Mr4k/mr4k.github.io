...
def weightedRandom(weights):
	...
	:param weights: A dictionary of weights (must all sum to one)
...
**Now why can we always do this?** Now that we have the algorithm we can use induction to give a formal proof. Our claim is a little more general from what we have been saying earlier but it will make the proof easier.  
<br>
